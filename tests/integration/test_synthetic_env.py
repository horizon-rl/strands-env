...
"""Integration tests for AgentWorldModelEnv with a real SGLang model.
...
@pytest.fixture
async def agent_world_model_env(data_dir, model_factory):
    """Create, reset, and yield an AgentWorldModelEnv; clean up after."""
...
class TestAgentWorldModelEnvStep:
    async def test_step_completes(self, agent_world_model_env):
        ...
        result = await agent_world_model_env.step(action)
    ...
    async def test_step_produces_reward(self, agent_world_model_env):
    ...
    async def test_step_produces_token_observation(self, agent_world_model_env):
    ...
    async def test_step_metrics(self, agent_world_model_env):
    ...
    async def test_final_response(self, agent_world_model_env):
    ...
    async def test_tool_iteration_limit(self, data_dir, model_factory):
        ...
        config = AgentWorldModelEnvConfig(scenario=SCENARIO_NAME, task_idx=0, data_dir=data_dir)
        env = AgentWorldModelEnv(model_factory=model_factory, config=config, max_tool_iters=1)
        await env.reset()
        try:
            action = Action(
                message=(
    ...
    async def test_reward_complete_when_db_intact(self, agent_world_model_env):
    ...
    async def test_get_user_task(self, data_dir, model_factory):
        ...
        config = AgentWorldModelEnvConfig(scenario=SCENARIO_NAME, task_idx=1, data_dir=data_dir)
        env = AgentWorldModelEnv(model_factory=model_factory, config=config)
        await env.reset()
        try:
